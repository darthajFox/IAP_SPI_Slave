...
*/
extern DPUCHAR *image_buffer; /* Pointer to the image in the system memory */
...
*/
DPULONG current_block_address=0U;    // current_block_start_address
DPUCHAR current_var_ID=Header_ID;    // current_block_ID
...
DPUCHAR* dp_get_data(DPUCHAR var_ID, DPULONG bit_index)
{
    ...
    if ((current_block_address == 0U) && (var_ID != Header_ID))  /// if requested block is missing in data file
    {
        return_bytes = 0U;
    }
    else
...
void dp_get_page_data(DPULONG image_requested_address)
{
    ...
    */
    ...
    #ifdef ENABLE_UART_HOSTLOADER
    {
        DPUCHAR addressOffset[4]={0x0, 0x0, 0x0, 0x0};
            ...
            MSS_UART_polled_tx (gp_my_uart, "D", 1);
            ...
            MSS_UART_polled_tx (gp_my_uart, addressOffset, 4);
            while(MSS_UART_tx_complete(gp_my_uart) == 0);    
            ...
            MSS_UART_polled_tx (gp_my_uart, numBytes, 4);
            while(MSS_UART_tx_complete(gp_my_uart) == 0);
            ...
            while (rx_size < 4)
            {
                rx_size += MSS_UART_get_rx(gp_my_uart, &numBytes[rx_size],4);
            }
            ...
            {
                rx_size += MSS_UART_get_rx(gp_my_uart, &page_global_buffer[rx_size],return_bytes);
            }
        ...
        */
        if (current_var_ID != requested_var_ID)
        {
            ...
            if (requested_var_ID != Header_ID)
            {
                ...
                header_size = dp_get_header_bytes((HEADER_SIZE_OFFSET),1U);   /// header size
                image_size = dp_get_header_bytes(IMAGE_SIZE_OFFSET,4U);       /// image size
                
                /* The last byte in the header is the number of data blocks in the dat file */
                num_vars = (DPUINT) dp_get_header_bytes(header_size-1U,1U);
                
                for (var_idx=0U; var_idx<num_vars; var_idx++)
                {
                    variable_ID = (DPUCHAR) dp_get_header_bytes(header_size+BTYES_PER_TABLE_RECORD*var_idx,1U);  /// data block ID
                    if (variable_ID == requested_var_ID)
                    {
                        current_block_address = dp_get_header_bytes(header_size+BTYES_PER_TABLE_RECORD*var_idx+1U,4U); /// data block start addr
    ...
    */
    DPUCHAR * dp_get_data_block_element_address(DPULONG bit_index)      /// get byte address by bit index within selected data file block 
    {
