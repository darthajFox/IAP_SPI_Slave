...
{
    logSrcMessage("Start", "dp_top_g5m");
...
void dp_G5M_program_action(void)
{
    ...
    if (error_code == DPE_SUCCESS)
    {
        ...
        {
            /* Global unit1 is used to hold the number of components */
            global_uint1 = (DPUINT)dp_get_bytes(GXM_Header_ID, GXM_DATASIZE_OFFSET, GXM_DATASIZE_BYTE_LENGTH);  /// number of bitstream components
            global_uint2 = 1u;
...
void dp_G5M_read_device_certificate_action(void)
{
    ...
    void dp_G5M_process_data(DPUCHAR BlockID)
    {
        DPUCHAR tmp_buf;
        #ifdef ENABLE_DISPLAY
        /* Global uint1 is used to hold the number of components */
        ...
        DataIndex = 0u;
        for (; global_uint2 <= global_uint1; global_uint2++)  /// component loop
        {
            ...
            g5_component_type = (DPUCHAR) dp_get_bytes(GXM_datastream_ID,G5M_COMPONENT_TYPE_IN_HEADER_BYTE+DataIndex/8,1);       /// determine component type
            ...
            for (global_ulong2 = 1u; global_ulong2 <= global_ulong1; global_ulong2++)  /// component blocks loop
            {
                #ifdef ENABLE_DISPLAY
    ...
    void dp_G5M_read_certificate(void)
    {
        ...
        if (cert_result & 0x2u == 0x1u)// error bit
        {  
            #ifdef ENABLE_DISPLAY
