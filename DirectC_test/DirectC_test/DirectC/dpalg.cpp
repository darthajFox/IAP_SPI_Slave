...
DPUCHAR Action_code;        // action to perform
...
DPUCHAR dp_top (void)
{
    ...
    {
        logSrcMessage("Action wasn't performed", "dp_top");
        error_code = DPE_ACTION_NOT_FOUND;
    }

    logSrcMessage("Stop", "dp_top");
    return error_code;
}
...
void dp_check_device_ID(void)
{
    ...
    * Therefore, it can be used to store the Data file ID for */
    DataIndex = dp_get_bytes(GXM_Header_ID, GXM_ID_OFFSET, GXM_ID_BYTE_LENGTH);  // read device ID
    
    
    global_ulong1 = dp_get_bytes(GXM_Header_ID, GXM_ID_MASK_OFFSET, 4U);   // read device ID Mask
