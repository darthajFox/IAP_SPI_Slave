...
DPUCHAR numBytes[4]={0x0, 0x0, 0x0, 0x0};
    ...
    void dp_report_progress(DPUCHAR value)
    {
        ...
        MSS_UART_polled_tx (gp_my_uart, "R", 1);
        while(MSS_UART_tx_complete(gp_my_uart) == 0);    
        /* Send the number of bytes */
        MSS_UART_polled_tx (gp_my_uart, &value, 1);
        while(MSS_UART_tx_complete(gp_my_uart) == 0);    
        #else
    ...
    {
        logSrcMessage(text, "dp_disp_text"); 
    }
    ...
    {        
        if (descriptive == HEX)
        {
            ...
            logSrcMessage(QString("%1").arg(value, 0, 16), "dp_disp_val"); 
        }
        else if (descriptive == CHR)
        {
            //length = int_to_chr_int( value, value_text);
            logSrcMessage(QString((char)value), "dp_disp_val"); 
        }
        else
        {
            //length = int_to_dec_int( value, value_text);
            logSrcMessage(QString("%1").arg(value, 0, 10), "dp_disp_val");
        } 
    }
    ...
    void dp_check_image_crc(void)
    {
        ...
        if ( (global_ulong1 == 0x69736544u) || (global_ulong1 == 0x65746341u) || (global_ulong1 == 0x2D4D3447u) || (global_ulong1 == 0x34475452u) || (global_ulong1 == 0x2D4D3547u))
        {
            ...
            else 
            {
                ...
                /* Global_uint1 is used to hold the value of the calculated CRC */
                global_uint1 = 0u;
